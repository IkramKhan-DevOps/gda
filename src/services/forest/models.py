...
class Wildlife(models.Model):
    ...
    types = models.ManyToManyField('WildlifeType', related_name='type', blank=True,
                                   help_text='Select the type of wildlife eg birds, animals etc')
...
class WildlifeType(models.Model):
    ...
    image = ResizedImageField(
        size=[800, 600], quality=75, upload_to='forest/wildlife/type', help_text='size of image must be 800*600',
        null=True
    )
...
class Greenery(models.Model):
    ...
    scientific_name = models.CharField(max_length=255, help_text='Scientific name of the plant,tree or flower etc',
                                       null=True)
    image = ResizedImageField(
    ...
    types = models.ManyToManyField('GreeneryType', related_name='greenery', blank=True,
                                   help_text='Select the type of greenery')
...
class GreeneryType(models.Model):
    ...
    image = ResizedImageField(
        size=[800, 600], quality=75, upload_to='forest/greenery/type', help_text='size of image must be 800*600',
        null=True
    )
